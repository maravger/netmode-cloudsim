 ...
 */
public abstract class DatacenterBrokerAbstractExtended extends CloudSimEntity implements DatacenterBroker {
     ...
     */
    public DatacenterBrokerAbstractExtended(final CloudSim simulation, final String name) {
        super(simulation);
        if(!name.isEmpty()) {
            setName(name);
        }
...
//        setDatacenterList((Set<Datacenter>) evt.getData());
        LOGGER.info("{}: {}: List of {} datacenter(s) received.", getSimulation().clockStr(), getName(), datacenterList.size());
        requestDatacenterToCreateWaitingVms(false);
    }
     ...
     */
    public void setDatacenterList(final Set<Datacenter> datacenterList) {
